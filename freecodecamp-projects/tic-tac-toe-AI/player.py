...
class GeniusComputerPlayer(Player):
    ...
    def minimax(self, state, player):
        ...
        other_player = 'O' if player == 'X' else 'X' # the other_player

    # first, we eant to check if the previous move is a winner 
    # this is our base case
        if state.current_winner == other_player:
            # wu should return position and score because we need to keep tracking the score
            # for minimax to work
        ...
        for possible_move in state.available_moves():
            ...
            else: # but minimize the other_player
                if sim_score['score'] < best['score']:
                    best = sim_score # replace 
